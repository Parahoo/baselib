...
static BYTE *GetSystemDes3Key()
{
  static BYTE Key[] = "\x41\x57\x8f\xee\xba\x59\xd1\x6e\xd2\x67\xd2\x02\x65\x84\x64\xa9\x2b\x8a\x0d\xc4\xb8\x5c\x41\x4b";
  return Key;
}

void EncryptPassWord(char *pEncryptData, const char *pData, int Len)
{
  BYTE *Key = GetSystemDes3Key();
  des3_context KeyContext;
  des3_set_3keys(&KeyContext, Key);

  for(int i = 0; i <= Len/8; i++)
    des3_encrypt(&KeyContext, (BYTE *)(pData + 8*i), (BYTE *)(pEncryptData + 8*i));
}

void DecryptPassWord(char *pData, const char *pDesData, int Len)
{
  BYTE *Key = GetSystemDes3Key();
  des3_context KeyContext;
  des3_set_3keys(&KeyContext, Key);
  ...
  for(int i = 0; i < Len/8; i++)
    des3_decrypt(&KeyContext, (BYTE *)(pDesData + 8*i), (BYTE *)(pData + 8*i));
}
